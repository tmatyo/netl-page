...
import { ExpiringDomainsPropsType } from '@/types';
import { router } from '@inertiajs/vue3';

const props = defineProps<ExpiringDomainsPropsType>();
...
<template>
    <AdminLayout>
        <div class="my-3" v-if="expiringDomains.data.length > 0">
        <h1 class="font-size-3xl brand-text">
            {{ $t('expiring_domains') }} <small class="text-gray-700">({{ expiringDomains.total }})</small>
        </h1>
        <SearchBar :searchTerm="searchQuery ?? ''" @search="(searchQuery: string) => search(searchQuery)" />
            <Pagination
                :data="{ current_page: expiringDomains.current_page, last_page: expiringDomains.last_page }"
                :links="{
                    first_page_url: expiringDomains.first_page_url,
                    prev_page_url: expiringDomains.prev_page_url,
                    next_page_url: expiringDomains.next_page_url,
                    last_page_url: expiringDomains.last_page_url,
                }"
                class="mt-4"
            />
                ...
                <table class="w-full text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
                    ...
                    <tbody>
                        <tr
                            ...
                            v-for="d in expiringDomains.data"
                            :key="d.id"
                        >
